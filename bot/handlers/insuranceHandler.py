...
def list_all_policies(message):
    ...
    for provider in insurance_providers:
        markup.add(types.InlineKeyboardButton(provider['name'], callback_data=f"insurance_provider_{provider['id']}"))
...
@telebot_bot.callback_query_handler(func=lambda call: call.data.startswith('insurance_provider_'))
def show_categories(call):
    provider_id = int(call.data.split("insurance_provider_")[1])
    markup = types.InlineKeyboardMarkup()
    ...
    for category in filtered_categories:
        markup.add(types.InlineKeyboardButton(category['category_name'], callback_data=f"insurance_category_{provider_id}_{category['id']}"))
    
    # Adding a "Back to Providers" button
    markup.add(types.InlineKeyboardButton("Back to Providers", callback_data="insurance_back_to_providers"))
...
@telebot_bot.callback_query_handler(func=lambda call: call.data.startswith('insurance_category_'))
def show_products(call):
    provider_id, category_id = map(int, call.data.split("insurance_category_")[1].split("_"))
    ...
    for product in filtered_products:
        markup.add(types.InlineKeyboardButton(product['product_name'], callback_data=f"insurance_product_{product['id']}"))
    
    # Add "Back to Categories" and "Back to Providers" buttons
    markup.add(types.InlineKeyboardButton("Back to Categories", callback_data=f"insurance_back_to_categories_{provider_id}"))
    markup.add(types.InlineKeyboardButton("Back to Providers", callback_data="insurance_back_to_providers"))
...
# Step 4: Go back to providers
@telebot_bot.callback_query_handler(func=lambda call: call.data == "insurance_back_to_providers")
...
# Step 5: Go back to categories
@telebot_bot.callback_query_handler(func=lambda call: call.data.startswith("insurance_back_to_categories_"))
def back_to_categories(call):
    provider_id = int(call.data.split("_")[4])  # Adjusted index for the new prefix
    markup = types.InlineKeyboardMarkup()
...
# Step 6: Display insurance product details when selected
@telebot_bot.callback_query_handler(func=lambda call: call.data.startswith('insurance_product_'))
def insurance_product_selected(call):
    product_id = int(call.data.split("insurance_product_")[1])
    
    # Find the product in the insurance_products list
    product = next((p for p in insurance_products if p['id'] == product_id), None)
    
    if product:
        summary = product.get('description', "No summary available.")
        markup = types.InlineKeyboardMarkup()
        telebot_bot.send_message(call.message.chat.id, f"Insurance Product: {product['product_name']}\n\nSummary:\n{summary}", reply_markup=markup)